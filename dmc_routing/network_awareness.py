...
class NetworkAwareness(app_manager.RyuApp):
    ...
    def switch_features_handler(self, ev):
        """
            Initial operation:
            match: arp, actions: Controller
            match: ipv4, actions: Controller
            table-miss actions: Normal
        """
        datapath = ev.msg.datapath
        self.logger.info("switch:%s connected", datapath.id)
        self._set_arp_flow(datapath)
        self._set_ip_flow(datapath)
        self._set_default_flow(datapath)

    def _set_arp_flow(self, datapath):
        ...
        self.add_flow(datapath, PRIORITY_ARP, match, actions)

    def _set_ip_flow(self, datapath):
        ofproto = datapath.ofproto
        parser = datapath.ofproto_parser
        ...
        actions = [parser.OFPActionOutput(ofproto.OFPP_CONTROLLER,
                                          ofproto.OFPCML_NO_BUFFER)]
        self.add_flow(datapath, PRIORITY_IP, match, actions)

    def _set_default_flow(self, datapath):
        parser = datapath.ofproto_parser

        # install table-miss flow entry
        match = parser.OFPMatch()
        actions = []
        self.add_flow(datapath, PRIORITY_DEFAULT, match, actions)
    ...
    def k_shortest_paths(self, graph, src, dst, weight='weight', k=1):
        """
            Create K shortest paths of src to dst.
        """
    ...
    def get_topology(self, ev):
        ...
        # self.logger.info("NetworkAwareness's get_topology is called.")
    ...
    def _packet_in_handler(self, ev):
        ...
        # print("NetworkAwareness's packet_in handler is called.")
