...
class ShortestForwarding(app_manager.RyuApp):
    ...
    def __init__(self, *args, **kwargs):
        ...
        path = '/routing/rules/{switchid}'
        mapper.connect('controller', path,
                       controller=RestController, action='set_rule',
        ...
        path = '/routing/test'
        mapper.connect('controller', path,
                       controller=RestController, action='test',
    ...
    """Modified here."""
    def send_drop_flow(self, datapath, flow_info):
        parser = datapath.ofproto_parser
        actions = []
        match = parser.OFPMatch(
    ...
    """Modified here."""
    def install_drop_flow(self, datapaths, link_to_port, access_table, path,
                     flow_info, buffer_id, data=None):
        """
            Install flow entires for roundtrip: go and back.
            @parameter: path=[dpid1, dpid2...]
    ...
    def _packet_in_handler(self, ev):
        ...
        if isinstance(ip_pkt, ipv4.ipv4):
            ...
            """Modified here."""
            if not isinstance(icmp_pkt, icmp.icmp):
                # self.logger.info("We just handle ICMP packet.")
                return
...
class SwitchOfsList(dict):
    def __init__(self):
        super(SwitchOfsList, self).__init__()
    ...
    def get_ofs(self, dp_id):
        ...
        if dpid in self:
            ...
            msg = 'sw is not connected. : switchID=%s' % dp_id
...
class RestController(ControllerBase):
    ...
    def _set_rule(self, req, switchid):
        try:
            ...
            RestController._LOGGER.debug('invalid syntax %s', req.body)
            return Response(status=400)

        msgs = ''
        src_ip = rule[REST_SRC_IP]
        dst_ip = rule[REST_DST_IP]
